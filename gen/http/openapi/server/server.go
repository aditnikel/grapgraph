...
import (
	...
	openapi "github.com/aditnikel/grapgraph/gen/openapi"
...
func New(
	...
	return &Server{
		...
		Docs:                NewDocsHandler(e.Docs, mux, decoder, encoder, errhandler, formatter),
...
// MountIndexHandler configures the mux to serve the "openapi" service "index"
// endpoint.
...
// calls the "openapi" service "index" endpoint.
...
// MountDocsHandler configures the mux to serve the "openapi" service "docs"
// endpoint.
func MountDocsHandler(mux goahttp.Muxer, h http.Handler) {
	f, ok := h.(http.HandlerFunc)
	if !ok {
		f = func(w http.ResponseWriter, r *http.Request) {
			h.ServeHTTP(w, r)
	...
	mux.Handle("GET", "/docs", f)
}

// NewDocsHandler creates a HTTP handler which loads the HTTP request and calls
// the "openapi" service "docs" endpoint.
func NewDocsHandler(
	endpoint goa.Endpoint,
	mux goahttp.Muxer,
	decoder func(*http.Request) goahttp.Decoder,
	...
	var (
		encodeResponse = EncodeDocsResponse(encoder)
		encodeError    = goahttp.ErrorEncoder(encoder, formatter)
	)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithValue(r.Context(), goahttp.AcceptTypeKey, r.Header.Get("Accept"))
		ctx = context.WithValue(ctx, goa.MethodKey, "docs")
		ctx = context.WithValue(ctx, goa.ServiceKey, "openapi")
		var err error
		res, err := endpoint(ctx, nil)
		if err != nil {
